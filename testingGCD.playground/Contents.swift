...
/*Все действия по дефолту выполняются в main потоке, поэтому сначала загружается firstViewController, выставляются все его настройки вроде title, background color и по нажатию на кнопку он переходит во вторую SecondViewController и ждет пока загрузится фотография и только потом происходит сам переходит, но если там будет огромное видео или много фотографий и если интернет будет слабым, то будет казаться что приложение сломано, так как после нажатия на кнопку не будут происходит никакие действия. Для исправления этого мы создали глобальную очередь с высоким приоритетом и в ней получили данные асинхронно, а затем все это вернули в главную очередь. Теперь по нажатию на кнопку мы проваливаемся во второй SecondViewController  и только потом прогружаются данные в виде фотографий в данном случае  */
